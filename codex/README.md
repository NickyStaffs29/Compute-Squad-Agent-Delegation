...
The updater first selects its source and stops before installing anything if the source is not the one selected. By default that is the checkout on `main` tracking `origin/main`: it stops if the checkout is on another branch or tracks another upstream, refreshes it with `git pull --ff-only`, and stops unless it is then exactly `origin/main`, so no local commit rides along. With `--source-sha <commit>` it installs an approved commit instead, from any branch: it never pulls, and stops unless the checkout is exactly that commit. Either way it stops if the checkout has uncommitted changes to tracked files, checked before the pull and again after it. Whichever commit it selects, it renders the build from an export of that commit's own files, read from git's objects, with that commit's own renderer: an untracked file, an uncommitted edit, or an edit git status cannot see (skip-worktree, assume-unchanged) never reaches the install. Git replace refs (`refs/replace/*`) are ignored, so the commit is read as git stores it and never as another commit or file substituted under its name, and a commit holding any symlink is refused before anything is installed, since a symlink could read a file from outside the commit. It also stops before installing anything while `compute-squad@compute-squad-local` is registered from a directory other than its build, and names `codex plugin marketplace remove compute-squad-local` to clear it. It then reads `codex debug models`. With no saved choices it asks for them on a terminal; with no terminal, as under a scheduler, it exits 3 with `setup gap: no Codex model choices saved` and the `--review-models` command, and installs nothing. It never falls back to the release defaults on its own. With saved choices, when the catalog has changed since you chose, it asks on a terminal whether to review them and warns under a scheduler, keeping them either way. It then checks your models against the catalog and stops with `$CODEX_HOME` unchanged if one is retired, missing, or lacks its effort, naming it. Last it renders the build, installs the plugin, agents, and profiles from it, and compares what it installed with a fresh render of the source; any difference fails the update, which then never prints its success line. It prints each tier's model. Nothing switches a model mid-run: a running session keeps the skill and agents it loaded, and a run resumed in a new session uses the new models from its next stage. There is no separate `codex plugin update` command. For launchd, cron, or Task Scheduler, invoke `/bin/bash` with absolute paths to the updater, `CODEX_BIN`, `GIT_BIN`, and `CODEX_HOME`; schedulers do not reliably inherit an interactive shell's `PATH`. If `CODEX_BIN` is a runtime shim, include its interpreter's directory in that `PATH`, along with `python3`'s. On macOS, put these values in the LaunchAgent's `EnvironmentVariables` dictionary, use a per-user LaunchAgent, and do not place shell assignments or `$HOME` in launchd's `ProgramArguments`. Use the scheduler's native stdout/stderr log settings. The updater does not create a schedule itself.

`bash "$HOME/src/compute-squad/codex/update.sh" --check` (with `--source-sha <commit>` for an approved commit) changes nothing: no pull, no lock, no catalog read, and no Codex call but `codex plugin list --json`. It renders the selected commit (the approved one with `--source-sha`, else HEAD), exported from git's objects as an update exports it, with your saved choices into a temporary directory outside `$CODEX_HOME` and compares it with what Codex runs: that the plugin is installed and enabled from the local build and no second copy is, every file of the cached plugin in both directions (the manifest and the whole skill, its references and hooks), the seven agents, and the four profiles. Each must be a regular file, reached through no symlink, with the source's bytes and exactly its executable bits. It also reports a checkout that is not the approved commit or has uncommitted changes, a commit it cannot export (one holding a symlink), and a held update lock. It prints one `check: MISMATCH` line per difference, then `check: FAILED`, and exits 1; with no difference it ends on `check: OK` and exits 0.
