...
3. Verify the archive file exists and its contents match the original BEFORE truncating `COMPUTE_SQUAD_LOG.md` to empty. This truncate is one of the pipeline's two legitimate whole-file writes (the other is the PM's clear-on-PASS in `05-pm-accept.md`); every append elsewhere in the pipeline uses a shell heredoc instead. Never truncate before a verified archive. Never discard a prior or failed run.
