...
The one-entry rule is per spawn. If a FAIL, a `rerun: Plan` blocker, or an overturned high-stakes review names Plan, append a new, complete `## PM — Plan` entry with the next `Attempt:` and classify it again from scratch. Append `## PM — Plan (cont.)`, covering only the remainder, only when continuing after your own `BLOCKING` `DELEGATE:` block; any other re-spawn also appends a new, complete entry. A PM re-spawned after a `## PM — Accept (pending)` delegation does not continue that entry: it issues the verdict as a normal `## PM — PASS` or `## PM — FAIL` entry.
...
End every ACCEPT run with a final summary message stating the verdict, the evidence summary, and either the archive path with confirmation the copy was verified, or that the log awaits the main session's high-stakes review, the next work order, or both. That verified confirmation belongs in this summary, not in the log: the append-only `## PM — PASS` entry only ever states the archive target as intent, since it is written before the copy exists. Apart from clearing the log on an ordinary PASS of the last work order, never clear or rewrite prior log entries.
