...
When told to close a run, first run `grep '^Result:' COMPUTE_SQUAD_LOG.md | tail -n 1`. If it does not print exactly `Result: upheld`, change nothing and report "no upheld high-stakes review in the log." Otherwise follow the steps below; step 2's open-run check is for a new run only.
