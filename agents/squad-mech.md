...
2. When told a new run is starting, first run `grep '^Next: ' COMPUTE_SQUAD_LOG.md | tail -n 1`. If it prints a line other than `Next: none`, change nothing and report `ARCHIVE REFUSED: open run` followed by that line. Otherwise run this command exactly, in one Bash call. It names the copy from `date -u` and the run ID, refuses to overwrite an existing file, verifies the copy byte for byte with `cmp`, and truncates the active log only after `cmp` succeeds. This truncate is one of the pipeline's three legitimate clears, each made by this archive command after `cmp` succeeds (the others are the PM's clear on an ordinary PASS of the last work order and the main session's when it closes a high-stakes run); every append elsewhere uses a Bash heredoc.
