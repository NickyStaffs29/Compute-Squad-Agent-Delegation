...
| AC2 | not met: pre-existing | reproduced | a real run writes a fourth file, .export-index, that no path line names; `node bin/export --out /tmp/y` on the base commit 1a2b3c4 in a temporary git worktree writes it too |
- `npm test` -> exit 0; 8 passed
...
BLOCKER:
- needs-human: waive or re-scope AC2
- why: a real run writes .export-index, which the dry run does not print, and the base commit 1a2b3c4 writes it the same way.

## Decision
Timestamp: 2026-09-05T10:31:10Z
Type: waiver
Covers: AC2
User's words: "Waive AC2 for .export-index; that file is internal."

## Status
Timestamp: 2026-09-05T10:31:18Z
Run: 2026-09-05-export-dry-run
Mode: full
Worktree: /home/dev/app, branch main
Base: 1a2b3c4
Plan: r1, work order all
Grant: all revisions, full-mode request
Next: spawn squad-pm (ACCEPT) for the verdict
Stop: after the PM verdict

## PM — PASS
Timestamp: 2026-09-05T10:36:44Z
Agent: squad-pm (claude-fable-5-1)
Attempt: 1
High-stakes: no
...
|---|---|---|---|
| AC1 | met | reproduced | `node bin/export --dry-run --out /tmp/x` exits 0 and /tmp/x is absent; refutation, a dry run during a real run: the real run keeps its lock |
| AC2 | waived | reproduced | the user waived it by the ## Decision of 2026-09-05T10:31:10Z; a real run still writes .export-index, as on the base commit |
- `npm test` -> exit 0; 8 passed
- `node bin/export --dry-run --out /tmp/x` -> exit 0; printed 3 paths, /tmp/x absent
Regressions: none
Outside scope: none
