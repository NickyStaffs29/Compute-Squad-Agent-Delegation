...
Findings: 30; skeptics run: 10 (cap 10)
...
- UNREVIEWED src/server/auth/__tests__/reset.routes.test.js:88 (medium): test (b) checks the outbox length but not that the second email went to ada. Evidence: lines 88-97 assert outbox.length === 2 only.
...
- UNREVIEWED src/server/middleware/rateLimit.js:2 (medium): the global limiter still lets one IP request resets for many accounts. Evidence: rateLimit.js:2 allows 300 requests per IP per 5 minutes and is keyed by IP only.
- UNREVIEWED src/server/auth/__tests__/reset.routes.test.js:99 (medium): no test covers a request 59,999 ms after the first. Evidence: tests (a) and (b) advance 59 s and 60 s only.
- UNREVIEWED src/server/auth/reset.service.js:8 (low): the new constant has no comment naming its unit. Evidence: line 8 reads "const RESEND_COOLDOWN_MS = 60 * 1000;".
- UNREVIEWED src/server/auth/reset.service.js:12 (low): the function comment still says every known address gets an email. Evidence: lines 12-13 predate the cooldown branch.
- UNREVIEWED src/server/auth/reset.service.js:22 (low): the cooldown branch repeats the GENERIC_RESULT return instead of sharing one exit. Evidence: lines 17 and 22 both return GENERIC_RESULT.
- UNREVIEWED src/server/auth/reset.service.js:30 (low): hashToken is exported but only tests import it. Evidence: grep -rn hashToken src lists reset.service.js and the test file only.
- UNREVIEWED src/server/auth/__tests__/reset.routes.test.js:3 (low): the test file imports createFakeClock twice. Evidence: lines 3 and 5 both require '../../clock'.
- UNREVIEWED src/server/auth/__tests__/reset.routes.test.js:15 (low): setup() builds two accounts where the new tests use one. Evidence: lines 8-14 create ada and grace.
- UNREVIEWED src/server/auth/__tests__/reset.routes.test.js:60 (low): a test name says 'cooldown' for a test of the 200 body. Evidence: line 60's name and its body disagree.
- UNREVIEWED src/server/auth/__tests__/reset.routes.test.js:72 (low): test (a) leaves a console.log in place. Evidence: line 72 reads console.log(res.body).
- UNREVIEWED src/server/auth/__tests__/reset.routes.test.js:101 (low): test (b)'s name gives 60 s where the body advances 60000 ms. Evidence: line 101.
- UNREVIEWED src/server/db/store.js:24 (low): listResetTokens() copies the whole row array on every call. Evidence: store.js:24 returns rows.filter(...).
- UNREVIEWED src/server/clock.js:9 (low): advance() accepts a negative value. Evidence: clock.js:9-11 adds ms without a check.
- UNREVIEWED src/server/email/send.js:3 (low): sendEmail throws on a missing template but not on a missing token. Evidence: send.js:3-5.
- UNREVIEWED src/server/log.js:11 (low): event() throws a bare Error with no code. Evidence: log.js:11-13.
- UNREVIEWED src/server/auth/auth.routes.js:8 (low): the email check does not trim before testing the type. Evidence: auth.routes.js:8.
- UNREVIEWED src/server/app.js:14 (low): routes is a Map keyed by a template string that the file builds twice. Evidence: app.js:15 and :23.
- UNREVIEWED package.json:7 (low): the test script runs node --test with no file pattern. Evidence: package.json:7.
- UNREVIEWED CLAUDE.md:5 (low): the fake-clock rule does not name advance(). Evidence: CLAUDE.md:5.
