...
Deviations: none
For acceptance:
- F1: a dry run inside a real run's lock window
- F2: paths print in the order a real run writes them
...
Outside scope: none
Executor points:
- F1: ran a dry run while a real run held the lock -> the dry run exits 0 and the real run keeps its lock
