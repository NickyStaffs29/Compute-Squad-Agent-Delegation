...
Findings: 12; skeptics run: 12 (cap 10)
...
- REFUTED src/server/log.js:3 (medium): EVENT_CODES has no reset_cooldown_hit, so logging the cooldown would throw. Evidence: log.js:3 reads "const EVENT_CODES = Object.freeze(['reset_email_sent']);".
- REFUTED src/server/middleware/rateLimit.js:2 (medium): the global limiter still lets one IP request resets for many accounts. Evidence: rateLimit.js:2 allows 300 requests per IP per 5 minutes and is keyed by IP only.
